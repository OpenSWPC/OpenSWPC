        ...
        do k = k0, k1
            ...
            if (zs(k) < depth(1)) then
                ...
                do i = i0 , i1
                    ...
                    rho1 = rho0(nlayer) * ( 1 + 0.8 * xi(k, i, tbl_rmed(nlayer))) * Cv(k)**(-5)
                    vp1 =  vp0(nlayer) * ( 1 + xi(k, i, tbl_rmed(nlayer))) * Cv(k) 
                    vs1 =  vs0(nlayer) * ( 1 + xi(k, i, tbl_rmed(nlayer))) * Cv(k) 
                    qp1 =         qp0(nlayer)
                    qs1 =         qs0(nlayer)
                    ...
                    do l = 1, nlayer-1            
                        if (depth(l) <= zs(k) .and. zs(k) < depth(l+1)) then
                            ...
                            rho1 =(rho0(l) + (rho0(l+1) - rho0(l)) / (depth(l+1) - depth(l)) * (zs(k) - depth(l))) * Cv(k)**(-5)
                            vp1 = ( vp0(l) + ( vp0(l+1) -  vp0(l)) / (depth(l+1) - depth(l)) * (zs(k) - depth(l))) * Cv(k)
                            vs1 = ( vs0(l) + ( vs0(l+1) -  vs0(l)) / (depth(l+1) - depth(l)) * (zs(k) - depth(l))) * Cv(k)
                            ...
                            if (vp0(l) > 0 .and. vs0(l) > 0) then
                                call vcheck_sh(vs1, rho1, xi(k, i, tbl_rmed(l)), &
