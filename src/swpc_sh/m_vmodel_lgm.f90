        ...
        do k = k0, k1
            ...
            if (zs(k) < depth(1)) then
                ...
                if (zs(k) < 0.0) then
                    ...
                    rho1 = 1.0 * Cv(k)**(-5)
                    vp1  = Cv(k) * 1.5 ! neglect the Munk profile as it does not affect to SH waves
                    vs1  = 0.0
                    qp1  = 1000000.0
                    qs1  = 1000000.0
                ...
                do l=1, nlayer-1
                    if ( depth(l) <= zs(k) .and. zs(k) < depth(l+1) ) then
                        rho1 = ( rho0(l) + (rho0(l+1) - rho0(l)) * (zs(k) - depth(l)) / (depth(l+1) - depth(l))) * Cv(k)**(-5)
                        vp1  = ( vp0(l) + ( vp0(l+1) -   vp0(l)) * (zs(k) - depth(l)) / (depth(l+1) - depth(l))) * Cv(k)
                        vs1  = ( vs0(l) + ( vs0(l+1) -   vs0(l)) * (zs(k) - depth(l)) / (depth(l+1) - depth(l))) * Cv(k)
