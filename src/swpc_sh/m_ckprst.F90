...
contains
  ...
  subroutine ckprst__checkpoint(it)
    ...
    if( arrived_checkpoint ) then
      ...
      open( io_ckp, file=trim(fn_ckp), action='write', form='unformatted' )
  ...
  subroutine ckprst__restart( it )
    ...
    call std__getio( io_ckp )
    open( io_ckp, file=trim(fn_ckp), action='read', iostat=ierr, status='old', form='unformatted' )
  ...
  subroutine ckprst__filedelete()
    ...
    call std__getio(io)
    open(io, file=fn_ckp, action='write', iostat=ierr, form='unformatted', status='replace')
    write(io) it_minus
    close(io)
  ...
  subroutine checkpoint_fname( fn_ckp )
    ...
    fn_ckp = trim(cdir) //  '/' // trim(title) // '.checkpoint.'// trim(cid)
