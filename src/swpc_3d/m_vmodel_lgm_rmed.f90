...
contains
    ...
    subroutine vmodel_lgm_rmed(io_prm, i0, i1, j0, j1, k0, k1, xc, yc, zc, vcut, rho, lam, mu, Qp, Qs, bd)
        ...
        vmax = cc * dh / dt ! 0.95 is a safety coefficient
            ...
            if (zs(k) < depth(1)) then
                ...
                do j = j0, j1
                    do i = i0 , i1
                        ...
                        rho1 = rho0(nlayer) * ( 1 + 0.8 * xi(k, i, j, tbl_rmed(nlayer)))
                        vp1 = Cv(k) * vp0(nlayer) * ( 1 + xi(k, i, j, tbl_rmed(nlayer)))
                        vs1 = Cv(k) * vs0(nlayer) * ( 1 + xi(k, i, j, tbl_rmed(nlayer)))
                        qp1 =         qp0(nlayer)
                        qs1 =         qs0(nlayer)
                        ...
                        do l = 1, nlayer-1            
                            if (depth(l) <= zs(k) .and. zs(k) < depth(l+1)) then
                                ...
                                !! random media
                                rho1 = rho1 * ( 1 + 0.8 * xi(k, i, j, tbl_rmed(l)))
                                vp1  = vp1  * ( 1 + xi(k, i, j, tbl_rmed(l)))
                                vs1  = vs1  * ( 1 + xi(k, i, j, tbl_rmed(l)))


                                if (vp0(l) > 0 .and. vs0(l) > 0) then
                                    call vcheck(vp1, vs1, rho1, xi(k, i, j, tbl_rmed(l)), &
